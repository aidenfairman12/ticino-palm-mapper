...
against this checkpoint's output and compare to checkpoints/dinov2_mae/
...
def parse_export_args() -> argparse.Namespace:
    ...
    p.add_argument(
        ...
        help="Optional: reuse the stats already saved in an existing checkpoint (e.g. "
             "checkpoints/dinov2_mae/checkpoint_best.pt) instead of recomputing via "
    ...
    p.add_argument("--output", type=Path, default=Path("checkpoints/dinov2_vanilla/checkpoint_best.pt"))
    return p.parse_args()
